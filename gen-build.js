...
function pinyinSlide(s){var cards='';s.items.forEach(function(it,i){var tones=it.tones?it.tones.join(' '):'';var seq=[];if(it.tones)seq=it.tones.slice();else if(it.spell)seq=[it.spell];else if(it.eg)seq=[it.eg];else seq=[it.py];var seqAttr=JSON.stringify(seq).replace(/'/g,'&#39;');var spk='<button class="speaker pyspk" data-seq=\''+seqAttr+'\'>🔊</button>';cards+='<div class="pycard r" style="--i:'+i+'"><div class="pylead">'+esc(it.py)+'</div>'+(tones?'<div class="pytones">'+esc(tones)+'</div>':'')+(it.spell?'<div class="pyeg">'+esc(it.spell)+'</div>':'')+(it.eg?'<div class="pyeg">'+esc(it.eg)+'</div>':'')+spk+'</div>';});return'<section class="slide" data-name="拼音"><div class="tag">'+esc(s.label||'拼音')+'</div><div class="title r" style="--i:1;font-size:80px">'+esc(s.head||'拼音')+'</div><div class="pygrid">'+cards+'</div><div class="hint">点 🔊 听四个声调，跟着念</div></section>';}
